...
import { Shield, Clock, Wrench, AlertCircle, FileCheck, FileText, Download, ArrowRight, MessageSquare, AlertTriangle, MapPin, IndianRupee, Plus, Car } from 'lucide-react';
        ...
        <div className="bg-slate-900 border border-slate-800 rounded-3xl p-6 sm:p-8 mb-8 shadow-xl">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mt-8 pt-6 border-t border-slate-800 text-xs">
            <div className="bg-slate-950/60 p-3.5 rounded-xl border border-slate-800">
              ...
              <span className="text-xl font-bold text-emerald-400">{intelligence?.history_coverage_pct || 75}%</span>
            </div>
            <div className="bg-slate-950/60 p-3.5 rounded-xl border border-slate-800">
              <span className="text-slate-400 block mb-1">Maintenance Evidence</span>
              <span className="text-xl font-bold text-slate-200">{intelligence?.maintenance_evidence_rating || 'HIGH'}</span>
            </div>
            <div className="bg-slate-950/60 p-3.5 rounded-xl border border-slate-800">
              <span className="text-slate-400 block mb-1">Insurance Claims</span>
              <span className="text-xl font-bold text-rose-400">{intelligence?.verified_claims_count || 0} Verified</span>
            </div>
            <div className="bg-slate-950/60 p-3.5 rounded-xl border border-slate-800">
              <span className="text-slate-400 block mb-1">Data Conflicts</span>
              <span className={`text-xl font-bold ${(intelligence?.data_conflicts_count || 0) > 0 ? 'text-rose-400' : 'text-slate-200'}`}>
                {intelligence?.data_conflicts_count || 0}
              </span>
            </div>
          </div>
        </div>
